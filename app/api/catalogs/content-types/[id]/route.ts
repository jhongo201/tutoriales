...
export const PUT = withAuth(async (req, { params }) => {
  try {
    ...
    await db.request()
      ...
      .query(`
        UPDATE tutorial_content_types SET
    ...
    console.error('Error PUT /api/catalogs/content-types/[id]:', error)
...
export const DELETE = withAuth(async (_req, { params }) => {
  try {
    ...
    const current = await db.request()
      .input('id', sql.Int, id)
      .query(`SELECT code FROM tutorial_content_types WHERE id = @id`)
    ...
    const used = await db.request()
      ...
      .query(`SELECT COUNT(*) AS total FROM tutorials WHERE content_type = @code`)
    ...
    await db.request()
      .input('id', sql.Int, id)
      .query(`DELETE FROM tutorial_content_types WHERE id = @id`)

    return NextResponse.json({ message: 'Eliminado' })
  } catch (error) {
    console.error('Error DELETE /api/catalogs/content-types/[id]:', error)
